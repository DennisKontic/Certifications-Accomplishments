<div align="center">

# 🛡️ My TryHackMe Journey

### 🏆 Ranked in the Top 1% Globally
*Executing Blue Team Operations & Defense Evasion scenarios.*

<br>

<a href="https://tryhackme.com/p/DonkeySec">
  <img src="https://tryhackme-badges.s3.amazonaws.com/DonkeySec.png" alt="TryHackMe Badge">
</a>

</div>

---

### 📊 Performance Statistics

I actively maintain a ranking within the **Top 1%** of the platform, focusing heavily on Cloud Security, SOC Operations, and Incident Response.

| Global Rank | Completed Rooms | Total Badges | Current Role |
| :---: | :---: | :---: | :---: |
| 🏆 **18,163** | **134** | **18** | 🛡️ **Defender** |

---

### 📜 Professional Certifications

#### 🎓 Security Analyst Level 1 (SAL1)
*Issued: March 12, 2025*
* **Core Skills:** Cyber Defense Frameworks, Threat Detection, Alert Triage, Case Reporting.
* **Featured Scenarios:**
    * *SOC Chaos B2*
    * *LockDown At Dark B1*
    * *Security Analyst Fundamentals*

---

### ☁️ Path Completions

| Path | Focus Areas | Date Issued |
| :--- | :--- | :--- |
| **Attacking and Defending AWS** | Compromising EC2, Reducing Policy Privileges, Abusing Lambda, Enumerating IAM. | *May 11, 2025* |
| **Defending Azure** | KQL for Threat Detection, Sentinel Monitoring, Defender for Endpoint. | *April 25, 2025* |
| **SOC Level 1** | SIEM Incident Handling, Endpoint Monitoring, Web Attacks. | *August 14, 2024* |

---

### 🎖️ Badge Collection

I have earned **18 Badges**, including several **Epic** and **Rare** achievements for completing specific challenges and maintaining consistency.

#### 🛡️ SOC & Simulation Badges (Rare/Epic)
| Badge | Description | Rarity |
| :--- | :--- | :--- |
| **1st Tabletop Completed** | Completed a tabletop exercise | 🟣 **Epic (0.2%)** |
| **Five Scenarios Completed** | Completed 5 full SOC scenarios | 🟣 **Epic (0.3%)** |
| **Boogeyman Slayer** | Finished the SOC L1 Capstone Module | 🔵 Rare (1.5%) |
| **100% True Positive Rate** | Perfect accuracy in a scenario | 🔵 Rare (3.7%) |
| **First Alert Closed** | Closing the first alert in a sim | 🔵 Rare (5.4%) |

#### 🦈 Analysis & Network Skills
| Badge | Description | Rarity |
| :--- | :--- | :--- |
| **Phish Hunter** | Completed the Phishing Analysis series | 🔵 Rare (4.2%) |
| **Wireshark** | Competent with Wireshark & Packet Analysis | 🔵 Rare (4.0%) |
| **Packet Master** | Finishing the TShark Exercises | 🔵 Rare (2.2%) |

#### ⚔️ Challenge Rooms
* **To Benign or Not Benign?** (Rare 2%)
* **Manic Monday** (Rare 2%)
* **Friday Fixer** (Rare 3.7%)
* **Skilled Navigator** (Rare 5.8%)
* **Handle the Disgruntled** (Rare 1.7%)

#### 📅 Milestones & Streaks
* **Gold League:** 1st Place in Gold League
* **30 Day Streak:** Hacking for 30 days solid
* **7 Day Streak:** Achieving a 7 day streak
* **First Four:** Completed 4 rooms in the first week
* **2025 ReCapMe:** Yearly activity summary

---

<div align="center">

**[ ⬅️ Return to Main Profile ](https://github.com/DennisKontic/Certifications-Accomplishments)**

</div>
